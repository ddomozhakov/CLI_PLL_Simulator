...
void screenSet(int **screen, struct buffer_wave *fi, int y_n, int x_n,
               int y_range) {
  for (int i = 0; i < x_n; i++) {
    if (((int)((fi[i].y) * 0.5 * (y_n / y_range)) < y_n) &&
        ((int)((fi[i].y) * 0.5 * (y_n / y_range)) >= 0)) {
...
void fieldSet(struct buffer_wave *fi, int x_n, int x_range, int y_n,
              int y_range) {
